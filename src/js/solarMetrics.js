...
export async function querySolarMetrics(paramData) {
    ...
    let pointDayTime = parseInt(currDate.toISOString().substring(2,4) + "090000")
    ...
    if (result.errors != null) {
        ...
        /*
        console.log("result.data Totals = "+result.data.totals.items.length)
        console.log("result.data yearTotals = "+result.data.yearTotals.items.length)
        */
...
// Query the database for menu and file information and store in js variables
//------------------------------------------------------------------------------------------------------------
export async function querySolarMetrics2(paramData) {
    let currDate = new Date()
    let pointDate = addDays(currDate, -1)
    let pointDateStartBucket = getDateInt(pointDate)
    // Starting at 7:00am on the current day
